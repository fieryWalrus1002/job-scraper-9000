...
1. **Prefilter** — Deterministic routing layer: country gate, local allowlist, obvious reject signals. No LLM cost for clear cases.
1. **Remote Filter Agent** — LLM classifies each remaining posting: genuinely remote-flexible vs. deceptive hybrid. Returns PASS/TRASH with rationale.
1. **Skills Fit Agent** — Scores PASS jobs against a versioned candidate profile on an ordinal 1-5 scale (technical overlap, level alignment, domain context). Returns `fit_score` (1-5), `confidence`, `score_rationale`, `top_matches`, `gaps`, and `hard_concerns`. `fit_score` is the verdict — there is no separate `verdict` field.
1. **Dispatch** — Delivers the ranked shortlist via email or a local FastAPI web UI.
