...
2. **Source from the existing scraping pipeline.** Do not construct synthetic records — they won't reflect real job posting language patterns.
3. **Review flow is unchanged.** All new records go through the Streamlit reviewer before being added to `ground_truth.jsonl`. The `_corrected` flag should be set when the human verdict differs from the teacher label.
4. **No re-review of existing records without cause.** If a human review was already done and `_corrected: false`, do not re-litigate it. Only re-review if the policy changes.
